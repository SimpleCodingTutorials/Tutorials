class Pacman extends Phaser.Scene {
  ...
  switchMode() {
    if (this.currentMode === "scared") {
        // After scared mode, revert back to the previous mode (scatter or chase)
        this.currentMode = this.previousMode || "scatter";  // Default to scatter mode
        this.setModeTimer(this[this.currentMode + "ModeDuration"]);

        this.ghostSpeed = this.speed*0.7;

        // Restore ghosts' paths to regular scatter or chase targets
        this.ghosts.forEach(ghost => {

          clearInterval(ghost.blinkInterval); 
            // Restore ghost visuals
        ...
        // Store the previous mode before switching to the next
        ...
        // Update paths for all ghosts based on the current mode
        this.ghosts.forEach(ghost => {
            let target = this.currentMode === "chase" 
                ? this.getChaseTarget(ghost) 
                : this.getScatterTarget(ghost);
  ...
  preload() {
    ...
    //added
    this.load.spritesheet("scaredGhost","ghost/gost afraid/spr_afraid_0.png",{
      frameWidth:32,frameHeight:32
    });

    this.load.spritesheet("scaredGhostWhite","ghost/gost afraid/spr_afraid_1.png",{
      frameWidth:32,frameHeight:32
    });
  ...
  // Switch to scared mode
  this.currentMode = "scared";
    
  // Update all ghosts' paths to be random or scared behavior
  this.setGhostsToScaredMode();
  
  // Set a timer to return to the previous mode after scared mode ends
  ...
  setGhostsToScaredMode() {
    this.ghosts.forEach(ghost => {
      // Here, define random paths or any behavior for scared mode
      let scaredTarget = this.getScaredTarget(); // A function for random movement
      this.updateGhostPath(ghost, scaredTarget);
      
      if(ghost.blinkInterval)
        clearInterval(ghost.blinkInterval); 

      const blinkTime = this.scaredModeDuration - 2000; // Blink in the last 2 seconds
      ...
      // Change ghost visuals, if needed

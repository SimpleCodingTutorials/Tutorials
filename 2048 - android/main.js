...
addSwipeEvents(gameBoard);

function addSwipeEvents(element) {
  let hammertime = new Hammer(element);
  hammertime.get("swipe").set({direction: Hammer.DIRECTION_ALL});
  hammertime.on("swipeleft",function(){swipe("left")});
  hammertime.on("swiperight",function(){swipe("right")});
  hammertime.on("swipeup",function(){swipe("up")});
  hammertime.on("swipedown",function(){swipe("down")});
}

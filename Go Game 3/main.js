...
function insertPiece(event) {
  ...
  if(captures.length>0) {
    if(!isValidCaptureMove(captures,clickedSquareCoordinates)) {
      ...
      captures.length = 0;
      let alertMessage = (iskoMove && clickedSquareCoordinates.every((v,i)=>v===koPoint[i])) ? "Move not allowed due to the Ko rule!" : "Self capture is not allowed!";
...
function isValidCaptureMove(captures,clickedSquareCoordinates) {
   ...
   return false;
   let turn = isBlackTurn ? 1 : 2;
   if(!allCapturedSameColor && iskoMove && clickedSquareCoordinates[0]==koPoint[0] && clickedSquareCoordinates[1]==koPoint[1]){
    return false;
   }
